...
import com.unuslumen.app.util.date.at
import com.unuslumen.app.util.date.now
import com.unuslumen.app.util.date.toDayOfWeek
...
class CalendarRepositoryImpl(
    ...
    /**
     * True only when both runtime grants the calendar provider demands are
     * currently held. Every provider access funnels through this check so an
     * ungranted state degrades to empty results instead of a SecurityException
     * crashing the calling coroutine. The UI layer is responsible for driving
     * the actual permission request; the repository just refuses quietly.
     */
    ...
    /**
     * Mutations (insert, update, delete, calendar creation) require
     * WRITE_CALENDAR. The event details screen drives the runtime request;
     * this check is the backstop so no callsite can ever throw a raw
     * SecurityException and kill the process.
     */
    ...
    override suspend fun getEvents(excludedCalendars: List<Int>, until: Long?): List<CalendarEvent> {
        if (!hasCalendarRead()) return emptyList()
    ...
    override suspend fun getEventById(id: Long): CalendarEvent? {
        if (!hasCalendarRead()) return null
        return withContext(ioDispatcher) {
            val projection = arrayOf(
    ...
    override suspend fun addEvent(event: CalendarEvent): Long? {
        if (!hasCalendarWrite()) return null
        return withContext(ioDispatcher){
