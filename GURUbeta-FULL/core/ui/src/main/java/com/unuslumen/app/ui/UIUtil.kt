...
import com.unuslumen.app.ui.theme.Green
import com.unuslumen.app.ui.theme.Orange
import com.unuslumen.app.ui.theme.Red
...
enum class StartUpScreenSettings(val value: Int, val screen: Screen) {
    ...
    LOBBY(1, Screen.LobbyScreen),
    NOTES(2, Screen.NotesScreen),
    JOURNAL(4, Screen.JournalScreen),
    BOOKMARKS(5, Screen.BookmarksScreen),
    CALENDAR(6, Screen.CalendarScreen),
    PORTAL(7, Screen.PortalScreen)
}

fun Int.toStartUpScreen(): StartUpScreenSettings {
    return StartUpScreenSettings.entries.firstOrNull { it.value == this } ?: StartUpScreenSettings.LOBBY
}

/**
 * The root navigation route this startup screen resolves to.
 *
 * Dashboard rides the Screen.Main shell because the dashboard lives inside
 * the bottom-bar graph with Lobby and Settings. Every other choice opens
 * directly as the NavHost's start destination, so it is the first room the
 * app enters and back-press exits the app from it.
 */
fun StartUpScreenSettings.rootRoute(): Screen = when (this) {
    StartUpScreenSettings.DASHBOARD -> Screen.Main
    StartUpScreenSettings.LOBBY -> Screen.LobbyScreen
    StartUpScreenSettings.NOTES -> Screen.NotesScreen
    StartUpScreenSettings.JOURNAL -> Screen.JournalScreen
    StartUpScreenSettings.BOOKMARKS -> Screen.BookmarksScreen
    StartUpScreenSettings.CALENDAR -> Screen.CalendarScreen
    StartUpScreenSettings.PORTAL -> Screen.PortalScreen
}
