...
The full app source is out now, published in this repo under AGPL-3.0. Below is the honest state of the work: everything is out and live.
...
**Done: the untangle.** User accounts and login flow are stripped from the app. The login gate and account state are gone, replaced by the bootstrap model connect on first launch.

**Done: licence and cleanup pass.** The AGPL-3.0 licence and trademark notice are on the app repo; legacy cruft was swept before the flip to public.

**Done: the repo is public.** Full source is here under AGPL-3.0, with a signed, checksummed APK on the Releases page. Sideloaded, no app store, no gatekeepers, every region at once.

**Done: the ecosystem wiring.** Prompt sections, skills (SKILL.md bodies pulled and installed), tool definitions (downloaded and registered on-device), agent mask templates, canvas packs and config defaults all sync against the publisher server. The keyword-driven memory injection layer runs fully on-device. Community submissions flow into the review pipeline: tools, skills, agents, packs.

**Done: the publisher server is live** for public sync, and the public catalog is up so anyone can browse everything published through the review pipeline: every prompt, every skill, every tool, every agent template, every canvas pack. Browsable by users and creators from outside the app, not just silent sync behind a screen. The server's *content* is public and browsable; its *source* stays private per the reasoning above.

**Straight about the launch state: public beta, honest.** The published source has not been security hardened yet, the audit pass on the permission boundaries, encryption and Tor routing is still ahead, and there are bugs sitting in corners the author's own use case never touches. The first users are the beta testers. That is not a caveat buried in fine print, it is the deal stated plainly: everyone who installs from here forward is helping find what a 10-month solo build could not catch alone, and the community bug-fixing loop starts on day one.
